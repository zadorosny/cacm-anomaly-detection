...
def dims_small():
    ...
    return gerar_dimensoes(
        n_agencias=5, n_associados=200, n_operadores=20,
        dt_inicio=hoje - timedelta(days=7), dt_fim=hoje, seed=7,
    )
...
def test_injetor_de_fraudes_marca_label():
    ...
    base = pd.DataFrame(
        {
            "transacao_id": ["t0"], "tipo": ["PIX_OUT"], "valor": [100.0],
            "dt_transacao": [pd.Timestamp("2026-05-01 12:00")],
            "conta_origem": ["CT0"], "conta_destino": ["CT1"],
            "associado_id": ["AS0000000"], "agencia_id": ["AG0000"],
            "operador_id": [None], "canal": ["APP"],
            "ip_origem": ["1.1.1.1"], "geo_lat": [-16.6], "geo_lon": [-49.3],
            "dt_ingestao": [pd.Timestamp.now()],
            "fraude_label": [False], "fraude_tipo": [None],
        }
    )
